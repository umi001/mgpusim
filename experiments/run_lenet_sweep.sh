#!/bin/bash
# LeNet interconnect sweep: 7 configs x 1 epoch
set -e
export PATH=$HOME/.local/go/bin:$HOME/go/bin:$PATH

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
RESULTS_DIR="$REPO_ROOT/experiments/results/lenet"
SAMPLE_DIR="$REPO_ROOT/amd/samples/lenet"
...
echo "LeNet sweep complete! Results in $RESULTS_DIR"
