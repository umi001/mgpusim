#!/bin/bash
# Minerva interconnect sweep: 7 configs x 1 epoch
set -e
export PATH=$HOME/.local/go/bin:$HOME/go/bin:$PATH

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
RESULTS_DIR="$REPO_ROOT/experiments/results/minerva"
SAMPLE_DIR="$REPO_ROOT/amd/samples/minerva"

mkdir -p "$RESULTS_DIR"
cd "$SAMPLE_DIR"

# Config: name bw latency
declare -a CONFIGS=(
...
echo "Minerva sweep complete! Results in $RESULTS_DIR"
